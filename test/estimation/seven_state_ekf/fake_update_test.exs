defmodule Estimation.SevenStateEkf.FakeUpdateTest do
  use ExUnit.Case
  require Logger
  ...
  test "Run full EKF cycle" do
    ekf = Estimation.SevenStateEkf.new()
    position = Common.Utils.LatLonAlt.new_deg(42, -120, 123)
    velocity = %{north: 1.0, east: 0 * 2.0, down: 0 * -3.0}

    ekf =
      Estimation.SevenStateEkf.update_from_gps(ekf, position, velocity)
      |> Estimation.SevenStateEkf.update_from_heading(0.1)
    ...
    ekf =
      Enum.reduce(1..2000, ekf, fn _x, acc ->
        ...
        acc = Estimation.SevenStateEkf.predict(acc, dt_accel_gyro)
    ...
    IO.inspect(ekf)
    IO.puts("position: #{Common.Utils.LatLonAlt.to_string(Estimation.SevenStateEkf.position_rrm(ekf))}")
