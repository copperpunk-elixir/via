...
defmodule Uart.Debug do
  use GenServer
  require Logger

  def start_link(config) do
    Logger.debug("Start Uart.Debug GenServer: #{inspect(config[:name])}")
  ...
  def init(config) do
    ...
    {:ok, %{data: [], uart_ref: nil, name: Keyword.fetch!(config, :name)}}
  end
  ...
  def handle_info({:circuits_uart, _port, data}, state) do
    Logger.debug("#{state.name}: #{inspect(data)}")
